...
static void smb380_work(struct work_struct *work)
{
	...
	mutex_lock(&sensor->lock);
	input_report_abs(sensor->idev, ABS_X, -1*sensor->data.y);
	input_report_abs(sensor->idev, ABS_Y, -1*sensor->data.x);
	input_report_abs(sensor->idev, ABS_Z, -1*sensor->data.z);
			...
			const struct i2c_device_id *id)
{
	...
	if (sensor->client->irq > 0) { /* interrupt */
		...
		err = request_irq(sensor->client->irq, 
			smb380_irq, IRQF_TRIGGER_HIGH,
				"smb380_wqirq", sensor);
		if (err < 0) {
